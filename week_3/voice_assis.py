...
class VoiceGenerator:
    def __init__(self, api_key):
        ...
        try:
            ...
            self.available_voices = {voice.name: voice.voice_id for voice in voices_response.voices}
            # Default voice mapping (fallback if API fails)
            self.default_voice_id = "21m00Tcm4TlvDq8ikWAM"  # Rachel's voice ID
        except Exception as e:
            print(f"Warning: Could not fetch voices from API: {e}")
            # Fallback to default voice IDs
...
class VoiceAssistantRAG:
    def __init__(self, elevenlabs_api_key):
        ...
        # Initialize Bedrock LLM with correct parameters
...
def main():
    ...
    if not all([elevenlabs_api_key, aws_bearer_token_bedrock]):
        st.error(
            "Please set ELEVEN_LABS_API_KEY and AWS_BEARER_TOKEN_BEDROCK in your environment variables"
        )
        return
    ...
    if page == "Setup Knowledge Base":
        ...
        try:
            ...
            if available_voices:
                # available_voices is now a dict {name: voice_id}, get list of names
                voice_names = list(available_voices.keys())
                selected_voice = st.sidebar.selectbox(
                    "Select Voice",
                    voice_names,
                    index=(
                        voice_names.index("Rachel")
                        if "Rachel" in voice_names
                        else 0
                    ),
                )
            else:
